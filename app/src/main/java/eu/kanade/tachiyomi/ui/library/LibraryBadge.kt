...
class LibraryBadge @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) :
    ...
    fun setUnreadDownload(unread: Int, downloads: Int, showTotalChapters: Boolean) {
        ...
        with(unread_text) {
            ...
            setTextColor(
                when {
                    unread == -1 && !showTotalChapters -> context.contextCompatColor(R.color.unread_badge) // this makes badge icon solid
            ...
            setBackgroundColor(
                when {
                    showTotalChapters -> context.contextCompatColor(R.color.total_badge)
                    else -> context.contextCompatColor(R.color.unread_badge)
                }
            )
            visibility = when {
